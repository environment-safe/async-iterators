...
let defaultLogger = new Logger({ level: 0 });
defaultLogger.registerChannel(makeConsoleChannel(console));

const Iterable = {
    from: (item, options={})=>{
        const logger = options.logger || defaultLogger;
        if(Array.isArray(item)){
            logger.log('converted to array iterator', Logger.INFO);
            let index = 0;
            const results = [];
            return (()=>{
                //let thisIndex = null;
                const res =  {
                    next() {
                        ...
                        if(options.autoincrement) index++;
                        logger.log(`next value: ${result.value}`, Logger.INFO);
                        return result;
                    },
        ...
        if(typeof item === 'object'){
            logger.log('converted to object iterator', Logger.INFO);
            let index = 0;
...
export class IteratorPool{
    constructor(iterator, options={pool:1}){
    ...
    async fill(){
        if(!this.done){
            for(let poolIndex=0; poolIndex < this.pool.length; poolIndex++){
                if(
                    (this.pool[poolIndex] && this.pool[poolIndex].finished) ||
                    (!this.pool[poolIndex])
                ){
    ...
    async next(){
        ...
        while(!this.cached[index]){ //cache until we have the right index
            await this.fill();
            if((this.cached[index] && this.cached[index].done) ||(
                this.done
            ) && this.cached.length === index) return {
                done:true, 
                results: ()=>this.iterator.results()
            };
        }
...
export const map = (object, options={}, callback, complete)=>{
    const logger = options.logger || defaultLogger;
    ...
    promise.next = async ()=>{
        ...
        logger.log(
            `Iteration ${result.current}/${result.count} on ${id} complete`, 
            Logger.INFO
        );
            ...
            while(!item.done){
                ...
                if(item.value !== undefined &&  item.current !== undefined){
                    transformedItem = await callback(item.value, item.current, async (value)=>{
                        if(rtrned) throw new Error(
                            'Cannot call done() after returning value'
                        );
                ...
                }else{ //flush if we got an empty row
                    await item.return({});
                    if(holdForDone) holdForDone();
                }
                if(transformedItem){
...
export const forEach = (iterable, callback, complete)=>{
    return map(iterable, {
        autoincrement: true
    }, callback, complete);
};

export const forAll = (iterable, callback, complete)=>{
    return map(iterable, {
        autoincrement: true
    }, callback, complete);
};

export const forEachBatch = (iterable, batchSize, callback, complete)=>{
    return map(iterable, {
        autoincrement: true,
        holdForDone: true,
        pool: batchSize
...
export const forEachEmission = forEach;
export const forAllEmissions = forAll;
export const forAllEmissionsInPool = forEachBatch;
